...
class SmokeTest(unittest.TestCase):
    ...
    def test_runtime_3jane_korean_report_request_uses_source_backed_evidence(self) -> None:
        ...
        self.assertIn("## 1. 결론 먼저", report)
        self.assertIn("## 2. 이 프로젝트가 하는 일", report)
        self.assertIn("## 3. 왜 지금 언급되는가", report)
        self.assertIn("## 4. 제품과 기술 확인", report)
        self.assertIn("## 5. 토큰/체인/가치 포착", report)
        self.assertIn("## 6. 팀/펀딩/KOL", report)
        self.assertIn("## 7. 리스크", report)
        self.assertIn("## 8. 다음 확인할 것", report)
        self.assertIn("## 9. 근거 링크", report)
