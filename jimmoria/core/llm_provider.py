...
from jimmoria.storage.paths import project_root
...
def _codex_sdk_prompt(request: LLMRequest) -> str:
    ...
    return (
        ...
        "Hermes-led personal multi-agent stack. You are not here to modify the repository for this task. "
...
def _claude_cli_prompt(request: LLMRequest) -> str:
    ...
    return (
        ...
        "Hermes-led personal multi-agent stack. Follow the system prompt and answer from the provided context."
        f"\nReasoning effort requested: {request.reasoning_effort.upper()}."
        f"{response_rule}\n\n"
        f"System prompt:\n{request.system_prompt}\n\n"
        f"User prompt:\n{request.user_prompt}\n"
