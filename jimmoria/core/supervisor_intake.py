...
def build_personal_agent_instruction_reply(
    applied: list[str],
    settings: PersonalAgentSettings,
...
def build_personal_agent_status_reply(settings: PersonalAgentSettings) -> list[str]:
    return [
...
def decide_supervisor_intake(line: str, settings: PersonalAgentSettings | None = None) -> SupervisorIntakeDecision:
    settings = settings or PersonalAgentSettings()
    stripped = line.strip()
    ...
    if not stripped:
        return SupervisorIntakeDecision(
            ...
            next_step="Wait for the next owner instruction.",
    ...
    if _looks_like_small_talk(stripped, lowered):
        return SupervisorIntakeDecision(
            ...
            needs_research_room=False,
            confidence=0.9,
            rationale="The user is greeting or casually talking to Hermes Agent.",
            next_step="Reply directly and wait for a concrete research, settings, or status request.",
            supervisor_authority=authority,
        )

    if _looks_like_personal_agent_status_request(stripped, lowered):
        return SupervisorIntakeDecision(
            intent_type="personal_agent_status",
            action="show_personal_agent_settings",
    ...
    if _looks_like_supervisor_chat(stripped, lowered):
        return SupervisorIntakeDecision(
            ...
            needs_research_room=False,
            confidence=0.84,
            rationale="The user is asking Hermes Agent a confirmation or operating question.",
    ...
    if _looks_like_source_only_request(stripped, lowered):
        return SupervisorIntakeDecision(
            ...
            needs_research_room=False,
            confidence=0.78,
            rationale="The user wants material stored or remembered, but chat input should not open a room unless a report is requested.",
    ...
    if _looks_like_report_creation_request(stripped, lowered):
        if not resolved.required_link_present:
            return SupervisorIntakeDecision(
                ...
                needs_research_room=False,
                confidence=0.88,
                rationale="The user explicitly requested a project dossier, but no source link was provided for Identity Gate.",
        ...
        return SupervisorIntakeDecision(
            ...
            confidence=0.86,
            rationale="The user explicitly asked JIMMORIA to create a report or dossier and supplied at least one source link.",
    ...
    if _looks_like_report_retrieval_request(stripped, lowered):
        return SupervisorIntakeDecision(
            ...
            needs_research_room=False,
            confidence=0.86,
            rationale="The user is asking to retrieve an existing report, not create a new dossier.",
    ...
    if stripped.startswith(("http://", "https://")):
        return SupervisorIntakeDecision(
            ...
            next_step="Ask whether the user wants a report, or use /add for source-only storage.",
            supervisor_authority=authority,
        )

    has_config = _has_any(stripped, lowered, PERSONAL_AGENT_CONFIG_TERMS) or _is_meta_instruction(stripped)
    ...
    if has_config and not has_explicit_research_action:
        return SupervisorIntakeDecision(
            intent_type="personal_agent_config",
            action="apply_personal_agent_instruction",
    ...
    if has_research or has_explicit_research_action:
        if not resolved.required_link_present:
            return SupervisorIntakeDecision(
                ...
                needs_research_room=False,
                confidence=0.82,
                rationale="The user asked for research-like work, but no source link was provided for Identity Gate.",
        ...
        return SupervisorIntakeDecision(
            ...
            needs_research_room=False,
            confidence=0.78,
            rationale="The user asked for research-like work and supplied source context, but did not explicitly request a report or dossier.",
    ...
    return SupervisorIntakeDecision(
        ...
        next_step="Ask the user what kind of work should be done before opening a room or saving settings.",
        supervisor_authority=authority,
    )


PERSONAL_AGENT_CONFIG_TERMS = [
    "설정",
    "세팅",
    "선택",
    "추가",
...
def _looks_like_personal_agent_status_request(original: str, lowered: str) -> bool:
    return _has_any(original, lowered, PERSONAL_AGENT_STATUS_TERMS)
...
def _looks_like_report_retrieval_request(original: str, lowered: str) -> bool:
    if _has_any(original, lowered, PERSONAL_AGENT_CONFIG_TERMS) or _is_meta_instruction(original):
        return False
...
def _looks_like_report_creation_request(original: str, lowered: str) -> bool:
    if _has_any(original, lowered, PERSONAL_AGENT_CONFIG_TERMS) and _is_meta_instruction(original):
        return False
