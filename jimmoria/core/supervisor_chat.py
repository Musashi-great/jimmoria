...
from jimmoria.core.supervisor_intake import SupervisorIntakeDecision
...
def supervisor_chat_system_prompt(settings: PersonalAgentSettings) -> str:
    return "\n".join(
        [
            ...
            f"User relationship: {settings.user_relationship}",
        ]
    )
...
def fallback_supervisor_chat_reply(line: str, settings: PersonalAgentSettings) -> list[str]:
    lowered = line.lower()
