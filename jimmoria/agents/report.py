...
from jimmoria.agents.reporting.evidence_ledger import (
...
from jimmoria.agents.reporting.quality import ReportQuality, assess_report_quality
...
from jimmoria.core.memory import FindingRecord, SharedMemory
from jimmoria.core.project_profile import find_project_profile_in_text
from jimmoria.core.room import ResearchRoom
from jimmoria.core.source_quality import is_relevant_source_url
...
class ReportAgent(BaseAgent):
    ...
    def run(self, room: ResearchRoom, memory: SharedMemory, bus: CollaborationBus, **kwargs: Any) -> AgentResult:
        ...
        personal_agent_settings = kwargs.get("personal_agent_settings")
        if not isinstance(personal_agent_settings, PersonalAgentSettings):
            personal_agent_settings = kwargs.get("company_settings")
        if not isinstance(personal_agent_settings, PersonalAgentSettings):
            personal_agent_settings = PersonalAgentSettings()
        ...
        finding = self.write_finding(
            ...
            data={
                ...
                "report_language": personal_agent_settings.report_language,
    ...
    def _write_llm_summary(
        ...
        personal_agent_settings: PersonalAgentSettings | None = None,
        *,
        company_settings: PersonalAgentSettings | None = None,
    ) -> str:
        if personal_agent_settings is None:
            personal_agent_settings = company_settings
        candidate_lines = []
        ...
        system_prompt = self.system_prompt(
            "You are the Report Agent inside a Hermes-led personal crypto research stack. "
        ...
        if personal_agent_settings is not None and personal_agent_settings.report_language == "ko":
...
def render_project_dossier(
    ...
    *,
    ...
    if personal_agent_settings is None:
        personal_agent_settings = company_settings
    settings = personal_agent_settings or PersonalAgentSettings()
...
def wants_korean_report(room: ResearchRoom, settings: PersonalAgentSettings) -> bool:
...
def write_representative_evidence_packet(
    *,
    ...
    if personal_agent_settings is None:
        personal_agent_settings = company_settings or PersonalAgentSettings()
    ...
    korean = personal_agent_settings.report_language == "ko" or wants_korean_report(room, personal_agent_settings)
