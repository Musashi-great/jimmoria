...
class SupervisorAgent(BaseAgent):
    ...
    def run(self, room: ResearchRoom, memory: SharedMemory, bus: CollaborationBus, **kwargs: Any) -> AgentResult:
        ...
        if not isinstance(personal_agent_settings, dict):
            personal_agent_settings = kwargs.get("company_settings") if isinstance(kwargs.get("company_settings"), dict) else {}
        ...
        supervisor_mode = personal_agent_settings.get("supervisor_mode", "research_director")
        summary = (
            "Research room initialized by Hermes personal-agent orchestration."
            if supervisor_mode in {"personal_agent_orchestrator", "company_ceo"}
    ...
    coordination_checkpoints = [
        {
            ...
            "purpose": "Confirm the owner request, output mode, quality bar, and whether a Research Room is needed.",
        },
        {
        ...
        },
        {
            ...
            "owner": "supervisor_agent",
            "purpose": "Review quality gate, delivery mode, unresolved risks, and owner-facing final output.",
        },
    ]
    return {
