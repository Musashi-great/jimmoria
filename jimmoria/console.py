...
from jimmoria import APP_NAME, __version__
...
from jimmoria.storage.run_store import list_run_summaries, load_run_file
...
class JimmoriaConsole:
    ...
    def print_agent_roster(self, *, active_only: bool = False) -> None:
    ...
    def print_company(self, *, active_only: bool = False) -> None:
        self.print_agent_roster(active_only=active_only)

    def print_personal_agent_settings(self, settings: Any, path: str | Path) -> None:
        ...
        lines = [
            ...
            f"User relationship: {getattr(settings, 'user_relationship', 'owner_operator')}",
            f"Auto-apply agent instructions: {getattr(settings, 'auto_apply_agent_instructions', True)}",
        ]
        if principles:
    ...
    def print_company_settings(self, settings: Any, path: str | Path) -> None:
        self.print_personal_agent_settings(settings, path)
    ...
    def print_personal_agent_settings_updated(self, settings: Any, applied: list[str], path: str | Path) -> None:
        lines = [
    ...
    def print_company_settings_updated(self, settings: Any, applied: list[str], path: str | Path) -> None:
        self.print_personal_agent_settings_updated(settings, applied, path)
