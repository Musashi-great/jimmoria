...
from jimmoria.agents.discovery import extract_project_query, project_identity_hints
from jimmoria.core.bus import CollaborationBus
from jimmoria.core.agent_spec import AgentSpecRegistry
from jimmoria.core.personal_agent_settings import personal_agent_settings_path_for, load_personal_agent_settings
from jimmoria.core.concurrency import ConcurrencyPolicy, load_concurrency_policy
from jimmoria.core.hook_registry import HookRegistry, runtime_event_to_hook_event
from jimmoria.core.hooks import HookEngine
from jimmoria.core.memory import FindingRecord, ProjectCandidate, SharedMemory, SourceRecord
from jimmoria.core.model_gateway import ModelGateway
from jimmoria.core.project_profile import find_project_profile
from jimmoria.core.process_spec import load_process_spec
from jimmoria.core.room import ResearchRoom
from jimmoria.core.runtime_state import RuntimeState
...
from jimmoria.core.usage import aggregate_llm_usage
from jimmoria.storage.json_store import save_memory
from jimmoria.storage.paths import resolve_project_path
from jimmoria.storage.run_store import append_report_index, delete_run_snapshot, save_run_snapshot
...
class ResearchRuntime:
    ...
    def run_article_research(
        ...
        *,
        ...
        personal_agent_settings = load_personal_agent_settings(personal_agent_settings_path_for(memory_path))
        ...
        try:
            self._run_agent(
                ...
                room,
                goals=room.goals,
                personal_agent_settings=personal_agent_settings.to_dict(),
            ...
            self._run_agent(
                ...
                personal_agent_settings=personal_agent_settings,
            )
            room.set_status(RuntimeState.SUPERVISOR_REVIEWING)
            self._run_supervisor_final_review(room, personal_agent_settings=personal_agent_settings)
    ...
    def _run_supervisor_final_review(self, room: ResearchRoom, *, personal_agent_settings: Any) -> None:
        self._emit(
            "final_review_start",
            room_id=room.room_id,
    ...
    def _append_supervisor_review_to_report(self, room: ResearchRoom, review: dict[str, Any], *, personal_agent_settings: Any) -> None:
        ...
        if not path.exists():
            return
        report_language = getattr(personal_agent_settings, "report_language", "en")
...
def _build_supervisor_final_review(room: ResearchRoom) -> dict[str, Any]:
    ...
    if insufficient:
        ...
        summary = "Hermes approved the report for owner delivery after council consensus and quality review."
        required_followups = []
        confidence = 0.8
    return {
