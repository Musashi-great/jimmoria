...
from jimmoria import APP_NAME
from jimmoria.connectors import register_default_connectors
from jimmoria.console import JimmoriaConsole, format_duration_ms, format_llm_usage
from jimmoria.runtime import ResearchRuntime
from jimmoria.runtime import DEFAULT_AGENTS
...
from jimmoria.core.supervisor_intake import (
    build_personal_agent_instruction_reply,
    build_personal_agent_status_reply,
    build_supervisor_reply,
)
from jimmoria.core.supervisor_brain import SupervisorBrain, SupervisorBrainTurn
from jimmoria.core.supervisor_chat import generate_supervisor_chat_reply
from jimmoria.core.supervisor_job_contract import build_supervisor_job_contract
from jimmoria.core.model_gateway import ModelGateway
from jimmoria.core.claude_models import (
    SUPPORTED_CLAUDE_MODEL_IDS,
    claude_model_by_index,
    supported_claude_model_lines,
)
from jimmoria.core.codex_models import (
    SUPPORTED_CODEX_MODEL_IDS,
    model_by_index,
    supported_codex_model_lines,
)
from jimmoria.core.grok_models import (
    SUPPORTED_GROK_MODEL_IDS,
    grok_model_by_index,
    supported_grok_model_lines,
)
from jimmoria.core.llm_provider import (
    claude_api_status,
    claude_cli_status,
    codex_api_status,
    codex_sdk_available,
...
from jimmoria.core.scheduler import CronRegistry, create_local_job
from jimmoria.core.thesis_memory import (
    OutcomeLabel,
    ThesisCard,
    ThesisMemoryStore,
    card_summary_line,
...
from jimmoria.core.workflow_loader import (
...
from jimmoria.storage.json_store import load_memory
from jimmoria.storage.paths import default_project_path, resolve_project_path, user_config_path
from jimmoria.storage.run_store import (
    events_after_seq,
    fork_run_snapshot,
    load_report_index,
    list_run_summaries,
...
MODEL_SETTING_ENV_NAMES = [
    ...
    "CLAUDE_CLI_COMMAND",
    "CLAUDE_CLI_TIMEOUT",
    "CLAUDE_MODEL_FAST_CHAT",
    "CLAUDE_MODEL_FAST",
    "CLAUDE_MODEL_REASONING",
    "CLAUDE_MODEL_WRITING",
    "CLAUDE_MODEL_STRONG",
    "ANTHROPIC_MODEL_FAST_CHAT",
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description=f"{APP_NAME} personal crypto research agent CLI.")
    ...
    workflow_parser = subparsers.add_parser("workflow", help="Inspect and run YAML specialist workflows.")
...
def web_command(args: argparse.Namespace) -> None:
    from jimmoria.web import run_web_server
...
def chat_command(args: argparse.Namespace) -> None:
    ...
    if os.getenv("LLM_PROVIDER") and not configured_model_provider_ready():
        print_screen(
            ...
            [
                "Saved model settings are not runnable in this shell.",
                model_readiness_summary(),
                "JIMMORIA will ask you to attach a working model before opening Research HQ.",
            ],
        )
    ...
    while True:
        ...
        settings_path = personal_agent_settings_path_for(args.memory)
        settings = load_personal_agent_settings(settings_path)
        ...
        if intake_decision.intent_type == "personal_agent_config":
            pending_report_creation_line = ""
            applied = apply_personal_agent_instruction(route_line, settings)
            save_personal_agent_settings(settings, settings_path)
            if supervisor_brain.memory_store.observe_user_message(route_line, settings):
                supervisor_brain.memory_store.save()
            reply = build_personal_agent_instruction_reply(applied, settings, settings_path)
        ...
        if intake_decision.intent_type == "personal_agent_status":
            pending_report_creation_line = ""
            reply = build_personal_agent_status_reply(settings)
            console.print_supervisor_reply(reply)
            console.print_personal_agent_settings(settings, settings_path)
            append_supervisor_history(supervisor_history, line, reply, brain=supervisor_brain, turn=supervisor_turn)
            continue
        ...
        if intake_decision.intent_type == "source_ingestion":
            try:
                ...
                if model_runtime_error_needs_setup(exc) and sys.stdin.isatty() and not args.skip_model_setup:
                    clear_model_provider_env()
                    ensure_interactive_model_ready(clear_before=True)
                append_supervisor_history(
                    supervisor_history,
                    line,
...
def handle_chat_command(
    ...
    if command in {"/agents", "/company"}:
        console.print_agent_roster(active_only=False)
        return False, last_room_id
    ...
    if command in {"/settings", "/company-settings"}:
        settings = load_personal_agent_settings(personal_agent_settings_path_for(args.memory))
        console.print_personal_agent_settings(settings, personal_agent_settings_path_for(args.memory))
        return False, last_room_id
...
def apply_personal_agent_instruction(line: str, settings: PersonalAgentSettings) -> list[str]:
    applied: list[str] = []
    lowered = line.lower()
    ...
    if any(term in line for term in personal_stack_terms):
        ...
        settings.user_relationship = "owner_operator"
    ...
    if any(term in line for term in ["설정 변경", "자체 반영", "아닐경우", "그러지말고", "출력하는게 달라", "입력하는거에 따라서"]):
        settings.auto_apply_agent_instructions = True
        ...
        settings.intake_policy["personal_agent_config"] = "apply settings directly without report generation"
        settings.intake_policy["personal_agent_status"] = "show personal-agent status/settings panel without report generation"
...
def ensure_interactive_model_ready(*, clear_before: bool = True) -> None:
    ...
    while not configured_model_provider_ready():
        ...
        print_screen(
            "Model Setup Required",
            [
                "No runnable live model is attached yet.",
                model_readiness_summary(),
                "Attach Codex, Grok, or Claude with OAuth/local login. API-key setup is legacy fallback only.",
            ],
        )
...
def configure_model_panel(*, clear_before: bool = True) -> None:
    ...
    if choice in {"hybrid", "dual", "multi", "codex_grok", "grok_codex", "codex+grok"}:
        configure_all_logged_in_models()
        print_current_model_config()
        return
...
MODEL_FAMILY_ORDER = ("codex", "grok", "claude")


def attached_model_families() -> list[str]:
    families = []
    raw = os.getenv("JIMMORIA_MODEL_FAMILIES", "")
    for item in raw.split(","):
        family = normalize_model_family(item)
        if family and family not in families:
            families.append(family)

    provider = (os.getenv("LLM_PROVIDER") or "").strip().lower().replace("-", "_")
    inferred: list[str] = []
    if provider in {"codex_sdk", "codex_cli", "codex_api", "codex"}:
        inferred = ["codex"]
        ...
        inferred = ["claude"]
    elif provider in {"codex_grok", "grok_codex"}:
        inferred = ["codex", "grok"]
    elif provider in {"multi", "hybrid", "dual"} and not families:
        inferred = ["codex", "grok"]
    for family in inferred:
        if family not in families:
            families.append(family)
    return [family for family in MODEL_FAMILY_ORDER if family in families]


def normalize_model_family(value: str) -> str:
    normalized = value.strip().lower().replace("-", "_")
    if normalized in {"codex", "openai", "openai_codex", "codex_sdk", "codex_cli", "codex_api"}:
        return "codex"
...
def attached_model_family_label() -> str:
    families = attached_model_families()
    return ", ".join(families) if families else "none"


def attach_model_family(family: str) -> None:
...
def set_attached_model_families(families: list[str]) -> None:
    normalized = [family for family in MODEL_FAMILY_ORDER if family in {normalize_model_family(item) for item in families}]
    if not normalized:
        os.environ.pop("JIMMORIA_MODEL_FAMILIES", None)
        os.environ.pop("LLM_PROVIDER", None)
        return
    os.environ["JIMMORIA_MODEL_FAMILIES"] = ",".join(normalized)
    if len(normalized) > 1:
        os.environ["LLM_PROVIDER"] = "multi"
        return
    family = normalized[0]
    if family == "codex":
        os.environ["LLM_PROVIDER"] = preferred_codex_provider()
    elif family == "grok":
        os.environ["LLM_PROVIDER"] = "grok" if preferred_grok_auth_provider() == "api_key" else "xai_oauth"
    elif family == "claude":
        os.environ["LLM_PROVIDER"] = preferred_claude_provider()


def model_readiness_summary() -> str:
    return (
        f"Codex OAuth/local: {codex_oauth_status()} | "
        f"Grok OAuth: {grok_oauth_status()} | "
        f"Claude local: {claude_cli_status()}"
    )


def configure_all_logged_in_models() -> None:
    """Apply every OAuth/local authenticated model family without asking for API keys."""

    clear_legacy_model_session_env()
    codex_available = codex_oauth_provider_ready()
    grok_available = grok_oauth_configured()
    ...
    print_screen(
        ...
        [
            ...
            f"Grok OAuth: {grok_oauth_status()}",
            f"Claude local: {claude_cli_status()}",
            "Raw OAuth tokens are not saved in model_settings.json.",
        ],
    )
    if families:
        save_model_settings()
...
def configure_codex_oauth() -> None:
    ...
    provider = "codex_sdk" if codex_sdk_available() else "codex_cli"
    if provider == "codex_sdk":
        configure_codex_sdk()
    else:
        configure_codex_cli(prompt_for_auth=False)
...
def configure_codex_sdk() -> None:
    ...
    if not codex_provider_ready("codex_sdk"):
        ...
        print_screen(
            "Codex SDK not ready",
            [
                "Codex SDK/local auth is not runnable in this shell yet.",
                f"SDK package: {'available' if codex_sdk_available() else 'not installed'}",
                f"Login status: {codex_login_status()}",
                "Attach Codex again after logging in.",
            ],
        )
        return
...
def configure_codex_cli(*, prompt_for_auth: bool = True) -> None:
    ...
    if not codex_provider_ready("codex_cli"):
        ...
        print_screen(
            "Codex CLI not ready",
            [
                "Codex CLI is not runnable or not logged in in this shell.",
                codex_login_status(),
                "Install/login Codex, then attach again.",
            ],
        )
        return
    finish_model_attachment(
...
def configure_codex_api(*, prompt_for_key: bool = True) -> None:
    ...
    if not codex_provider_ready("codex_api"):
        clear_screen()
        print_screen(
            "Codex API not ready",
            [
                "Codex API needs OPENAI_API_KEY or CODEX_API_KEY in this shell.",
                codex_api_status(),
            ],
        )
        return
...
def configure_grok() -> None:
    ...
    if not grok_is_configured():
        ...
        print_screen(
            "Grok not ready",
            [
                "Grok/xAI needs a working OAuth session, OAuth token file, or OAuth token command.",
                grok_oauth_status() if preferred_grok_auth_provider() in {"xai_oauth", "grok_oauth"} else grok_auth_status(),
            ],
        )
        return
    finish_model_attachment(
...
def configure_model_routes(
    ...
    *,
    ...
    if choice in {"3", "same", "one"}:
        ...
        clear_model_route_env(prefix)
        if model and (
            model in supported_model_ids
            or (provider_label.startswith("Grok") and model.startswith("grok-"))
            or (provider_label.startswith("Claude") and model.startswith("claude-"))
        ):
...
def configure_model_routes_advanced(
    ...
    *,
    ...
    for tier, label in MODEL_ROUTE_TIERS:
        ...
        if selected and (
            selected in supported_model_ids
            or (provider_label.startswith("Grok") and selected.startswith("grok-"))
            or (provider_label.startswith("Claude") and selected.startswith("claude-"))
        ):
...
def grok_is_configured() -> bool:
    auth = preferred_grok_auth_provider()
    if auth in {"xai_oauth", "grok_oauth"}:
        return grok_oauth_configured()
    return "missing" not in grok_auth_status().lower()


def grok_oauth_configured() -> bool:
    return "missing" not in grok_oauth_status().lower()
...
def claude_is_configured() -> bool:
    auth = preferred_claude_auth_provider()
    if auth in {"claude_cli", "claude_oauth", "oauth", "cli"}:
        return claude_cli_available()
    return claude_api_key_configured()


def preferred_claude_auth_provider() -> str:
    configured = (os.getenv("JIMMORIA_CLAUDE_AUTH_PROVIDER") or "").strip().lower().replace("-", "_")
    if configured in {"api_key", "claude_api", "anthropic_api", "claude_cli", "claude_oauth", "oauth", "cli"}:
        return "claude_cli" if configured in {"claude_cli", "claude_oauth", "oauth", "cli"} else "api_key"
    if claude_cli_available():
        return "claude_cli"
    if claude_api_key_configured():
        return "api_key"
    return "claude_cli"


def preferred_claude_provider() -> str:
    return "claude_cli" if preferred_claude_auth_provider() == "claude_cli" else "claude_api"


def claude_provider_ready(provider: str) -> bool:
    normalized = provider.strip().lower().replace("-", "_")
    if normalized in {"claude_cli", "claude_oauth", "oauth", "cli"}:
        return claude_cli_available()
    if normalized in {"claude", "claude_api", "anthropic", "anthropic_api", "api", "api_key"}:
        return claude_api_key_configured()
    return claude_is_configured()


def claude_multi_auth_status() -> str:
    return f"{claude_api_status()} / {claude_cli_status()}"


def preferred_codex_provider() -> str:
    configured = (os.getenv("JIMMORIA_CODEX_PROVIDER") or os.getenv("CODEX_PROVIDER") or "").strip().lower()
    if configured in {"codex_sdk", "codex_cli", "codex_api"} and codex_provider_ready(configured):
        return configured
...
def codex_oauth_provider_ready() -> bool:
    return codex_provider_ready("codex_sdk") or codex_provider_ready("codex_cli")


def preferred_codex_oauth_provider() -> str:
    if codex_provider_ready("codex_sdk"):
        return "codex_sdk"
    return "codex_cli"


def codex_oauth_status() -> str:
    provider = preferred_codex_oauth_provider()
    provider_label = "Codex SDK" if provider == "codex_sdk" else "Codex CLI"
    return f"{provider_label}; {codex_login_status()}"
...
def configured_model_provider_ready() -> bool:
    ...
    if provider in {"claude", "anthropic", "claude_api", "anthropic_api", "claude_cli", "claude_oauth"}:
        return claude_provider_ready(provider)
    if provider in {"codex_grok", "grok_codex", "codex+grok", "grok+codex", "hybrid", "dual", "multi"}:
        families = attached_model_families()
        if not families:
            return False
        return all(model_family_ready(family) for family in families)
    return False


def model_family_ready(family: str) -> bool:
    normalized = normalize_model_family(family)
    if normalized == "codex":
        return codex_provider_ready(preferred_codex_provider())
    if normalized == "grok":
        return grok_is_configured()
    if normalized == "claude":
        return claude_provider_ready(preferred_claude_provider())
    return False
...
def model_runtime_error_needs_setup(exc: RuntimeError) -> bool:
    text = str(exc).lower()
    markers = [
        "not found on path",
